...
const Admin = () => {
  ...
  const loadUserAssignments = async (u) => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/admin/user-dashboards/${u.id}`,
        authHeader,
      );
  ...
  const submitResetPassword = async (e) => {
    ...
    try {
      await axios.post(
        `${API_URL}/admin/users/${resetTargetUser.id}/reset-password`,
        { newPassword: resetPassword },
        authHeader,
      );
      flash(
        `Password reset for ${resetTargetUser.email}. They'll be asked to change it on next login.`,
      );
  ...
  const submitDashboardForm = async (e) => {
    ...
    try {
      if (editingDashboardId) {
        await axios.put(
          `${API_URL}/admin/dashboards/${editingDashboardId}`,
          dashboardForm,
          authHeader,
        );
  ...
  const deleteDashboard = async (id) => {
    if (
      !window.confirm(
        "Delete this dashboard? This will remove it from all user assignments.",
      )
    )
      return;
    try {
  ...
  return (
    <div className="admin-page">
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.25rem" }}>
        <button
          className={`btn ${tab === "users" ? "btn-primary" : "btn-muted"}`}
          onClick={() => setTab("users")}
        >
          Users
        </button>
        <button
          className={`btn ${tab === "dashboards" ? "btn-primary" : "btn-muted"}`}
          onClick={() => setTab("dashboards")}
        >
          Dashboards
        </button>
        <button
          className={`btn ${tab === "audit" ? "btn-primary" : "btn-muted"}`}
          ...
          <div className="admin-card">
                    ...
                    >
                      <td>
                        ...
                        >
                          {u.full_name || u.email}
                          <br />
                          <small style={{ color: "var(--ims-gray-700)" }}>
                            {u.email}
                          </small>
                        </button>
                      </td>
            ...
            {selectedUser && (
              <div style={{ marginTop: "1.25rem" }}>
                <h3>
                  Dashboards for {selectedUser.full_name || selectedUser.email}
                </h3>
                <p
                  style={{ fontSize: "0.85rem", color: "var(--ims-gray-700)" }}
                >
                  Only checked dashboards will be visible to this user.
                  Unchecked = hidden.
                </p>
                {Object.keys(dashboardsByCategory).length === 0 && (
                  <p>
                    No dashboards exist yet — add some in the Dashboards tab.
                  </p>
                )}
          ...
          <div className="admin-card">
            ...
            <form onSubmit={submitUserForm}>
              ...
              {!editingUserId && (
                ...
                >
                  The user will be required to set their own password on first
                  login.
                </p>
              )}
            </form>
      ...
      {tab === "dashboards" && (
        ...
        >
          <div className="admin-card">
            <h2 style={{ marginTop: 0 }}>
              {editingDashboardId ? "Edit Dashboard" : "Add New Dashboard"}
            </h2>
            <form onSubmit={submitDashboardForm}>
              ...
              <input
                ...
                onChange={(e) =>
                  setDashboardForm({ ...dashboardForm, url: e.target.value })
                }
              />
              <input
                type="text"
                placeholder="Category (e.g., SEO, Sales)"
                className="admin-input"
                required
              ...
              <input
                ...
                className="admin-input"
                value={dashboardForm.icon}
                onChange={(e) =>
                  setDashboardForm({ ...dashboardForm, icon: e.target.value })
                }
              />
              <input
                type="text"
                ...
                className="admin-input"
                value={dashboardForm.backgroundColor}
                onChange={(e) =>
                  setDashboardForm({
                    ...dashboardForm,
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button type="submit" className="btn btn-primary">
                  {editingDashboardId ? "Save Changes" : "Add Dashboard"}
                </button>
                {editingDashboardId && (
                  <button
                    type="button"
        ...
        >
          <div className="admin-card" style={{ width: "min(400px, 90vw)" }}>
            <h3 style={{ marginTop: 0 }}>
              Reset Password for {resetTargetUser.email}
            </h3>
            <form onSubmit={submitResetPassword}>
              ...
              <p style={{ fontSize: "0.82rem", color: "var(--ims-gray-700)" }}>
                The user will be required to set their own password on next
                login.
              </p>
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button type="submit" className="btn btn-primary">
